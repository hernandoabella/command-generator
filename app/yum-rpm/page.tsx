...
export default function YumRpmPage() {
  const [action, setAction] = useState<'install' | 'remove' | 'update' | 'list' | 'info'>('install');
  ...
  useEffect(() => {
    ...
    switch (action) {
      case 'install':
        cmd = `sudo yum install ${packageName || '<package>'}`;
        break;
      case 'remove':
        cmd = `sudo yum remove ${packageName || '<package>'}`;
        break;
      case 'update':
        cmd = `sudo yum update ${packageName || ''}`.trim();
        break;
      case 'list':
        cmd = 'rpm -qa';
        break;
      case 'info':
        cmd = `rpm -qi ${packageName || '<package>'}`;
        break;
      default:
        cmd = '';
    }
      ...
      <main className="ml-0 lg:ml-64 p-6 md:p-10 w-full max-w-5xl mx-auto pt-20 lg:pt-10">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4 mb-8 border-b border-gray-700/50 pb-6">
          ...
          <h1 className="text-3xl font-extrabold bg-gradient-to-r from-white to-teal-400 bg-clip-text text-transparent leading-tight">
            YUM / RPM Command Generator
          </h1>
        </motion.div>

        {/* Action Selection */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-8">
          <button className={buttonClass(action === 'install')} onClick={() => setAction('install')}>Install</button>
          <button className={buttonClass(action === 'remove')} onClick={() => setAction('remove')}>Remove</button>
          <button className={buttonClass(action === 'update')} onClick={() => setAction('update')}>Update</button>
          <button className={buttonClass(action === 'list')} onClick={() => setAction('list')}>List Packages</button>
          <button className={buttonClass(action === 'info')} onClick={() => setAction('info')}>Package Info</button>
        </motion.div>

        {/* Package Input */}
        {(action === 'install' || action === 'remove' || action === 'info' || action === 'update') && (
          <div className="mb-6">
            <label className="block mb-2 text-gray-400 font-medium">Package Name</label>
            <input
              type="text"
              placeholder="e.g., nginx, curl"
              value={packageName}
        ...
        <div className="p-4 bg-gray-800 rounded-xl border border-gray-700 text-sm text-gray-400">
          <ul className="list-disc list-inside space-y-1">
            <li>Use <code>sudo</code> for commands requiring root privileges.</li>
            <li><code>yum update</code> updates package lists and upgrades installed packages.</li>
            <li><code>rpm -qa</code> lists all installed packages.</li>
            <li>For installing/removing packages, provide the exact package name.</li>
          </ul>
        </div>
      </main>
    </div>
