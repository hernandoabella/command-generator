...
type CommandType = 'journalctl' | 'dmesg' | 'tail';

export default function LogsDebuggingPage() {
  const [commandType, setCommandType] = useState<CommandType>('journalctl');
  ...
  useEffect(() => {
    ...
    if (commandType === 'journalctl') {
      if (lines) cmd += ` -n ${lines}`;
      if (filter) cmd += ` -u ${filter}`;
    } else if (commandType === 'dmesg') {
      if (lines) cmd += ` | tail -n ${lines}`;
    } else if (commandType === 'tail') {
      if (lines) cmd += ` -n ${lines}`;
      if (filter) cmd += ` ${filter}`;
    }

    setGeneratedCommand(cmd.trim());
  }, [commandType, lines, filter]);
    ...
    <div className="flex min-h-screen bg-gray-900 text-gray-100">
      ...
      <main className="ml-0 lg:ml-64 p-6 md:p-10 w-full max-w-5xl mx-auto pt-20 lg:pt-10">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-3 gap-3 mb-6">
          <button className={buttonClass(commandType === 'journalctl')} onClick={() => setCommandType('journalctl')}>journalctl</button>
          <button className={buttonClass(commandType === 'dmesg')} onClick={() => setCommandType('dmesg')}>dmesg</button>
          <button className={buttonClass(commandType === 'tail')} onClick={() => setCommandType('tail')}>tail</button>
        </motion.div>

        {/* Options */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            <label className="block mb-2 text-gray-400 font-medium">Number of lines (-n)</label>
            <input
              type="text"
              placeholder="50"
              value={lines}
          ...
          {(commandType === 'journalctl' || commandType === 'tail') && (
            <div>
              <label className="block mb-2 text-gray-400 font-medium">{commandType === 'journalctl' ? 'Service Name (-u)' : 'File Path'}</label>
              <input
                type="text"
                placeholder={commandType === 'journalctl' ? 'nginx.service' : '/var/log/syslog'}
                value={filter}
        ...
        <div className="p-4 bg-gray-800 rounded-xl border border-gray-700 text-sm text-gray-400">
          <ul className="list-disc list-inside space-y-1">
            <li><code>journalctl</code>: shows logs from the systemd journal. Use <code>-u service</code> to filter by service.</li>
            <li><code>dmesg</code>: shows kernel messages; can combine with <code>tail</code> to see the latest lines.</li>
            <li><code>tail</code>: shows the last lines of a log file; useful with <code>-f</code> to monitor in real-time.</li>
          </ul>
        </div>
      </main>
    </div>
